...
function Enemy(x, y, sensorRange, attackRange, attackStrength, life) {
    ...
    self.update = function () {
        ...
        for (i = 0; i < towers.length; i += 1) {
            var range = self.character.distanceTo(towers[i].character);
            if (range <= self.sensorRange) {
                closest.push({ tower : towers[i], range : range });
            }
        }
        if (closest.length === 0 && towers.length > 0) {
            self.character.moveToPoint = new Point(towers[0].character.entity.x, towers[0].character.entity.y);
            self.character.speed = 2;
        } else {
            closest.sort(function(a,b){return a.range-b.range;});
            for (i = 0; i < closest.length; i += 1) {
                if (closest[i].range <= self.attackRange) {
                    self.attack(closest[i].tower);
                    move = false;
                    break;
                } else {
                    self.character.moveToPoint = new Point(
                        closest[i].tower.character.entity.x,
                        closest[i].tower.character.entity.y);
                    self.character.speed = 2;
                    break;
                }
            }
    ...
    self.attack = function (tower) {
        ...
        self.flare.x += Math.cos(a) * 4;
        self.flare.y += Math.sin(a) * 4;
        self.flare.regX = 8;
        self.flare.regY = 8;
        if (!self.showFlare && self.flareTime <= 0) {
        ...
        self.character.entity.rotation = Math.atan2(
            tower.character.entity.y - self.character.entity.y,
            tower.character.entity.x - self.character.entity.x) * 180.0 / Math.PI;
        tower.character.life -= self.attackStrength * dt;
        tower.character.aggressors += 1;
    };
}
